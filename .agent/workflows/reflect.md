---
description: Reflect on session corrections and update ANTIGRAVITY.md
---
# Workflow: Antigravity Reflect

**IMPORTANT: Tool Mapping**
- You do NOT have `AskUserQuestion`. Instead, ask the user in the chat and wait for their response.
- You do NOT have `TodoWrite`. Use `sequential-thinking` to track your progress or write to `.reflect_progress.md`.
- You do NOT have `Edit`. Use `mcp_filesystem_edit_file` or `replace_file_content` or `multi_replace_file_content`.
- You do NOT have `Bash`. Use `run_command`.
- You do NOT have `Grep`. Use `grep_search`.
- `~/.antigravity/` locations: If `~/.antigravity/` does not exist or is not accessible, look for `.agent/` or ask the user where their Antigravity config is.
- `learnings-queue.json`: This file is populated by hooks. If it is empty or missing, rely on `--scan-history` logic (Step 0.5) to find learnings from conversation history.

**Original Instructions:**
---
description: Reflect on session corrections and update ANTIGRAVITY.md (with human review)
...
- `--dedupe`: Scan ANTIGRAVITY.md for similar entries and propose consolidations.
- `--include-tool-errors`: Include project-specific tool execution errors in scan (auto-enabled with `--scan-history`).

## Context
- Project ANTIGRAVITY.md: @ANTIGRAVITY.md
- Global ANTIGRAVITY.md: @~/.antigravity/ANTIGRAVITY.md
- Learnings queue: !`cat ~/.antigravity/learnings-queue.json 2>/dev/null || echo "[]"`
- Current project: !`pwd`

## Multi-Target Export

Claude-reflect syncs learnings to ANTIGRAVITY.md files (including subdirectories), skill files, and AGENTS.md.

**Supported Targets:**

| Target | File Path | Format | Notes |
|--------|-----------|--------|-------|
| **Global ANTIGRAVITY.md** | `~/.antigravity/ANTIGRAVITY.md` | Markdown | Always enabled |
| **Project ANTIGRAVITY.md** | `./ANTIGRAVITY.md` | Markdown | If exists |
| **Subdirectory ANTIGRAVITY.md** | `./**/ANTIGRAVITY.md` | Markdown | Auto-discovered |
...
Use the Python utility to find all ANTIGRAVITY.md files:
```python
import sys
sys.path.append(r'c:\Users\user\Documents\GitHub\scrivener-flow\my-case-tracker\.agent\skills\antigravity-reflect\scripts')
from lib.reflect_utils import find_antigravity_files
files = find_antigravity_files()  # Returns list of {path, relative_path, type}
```

Or discover manually:
```bash
# Find all ANTIGRAVITY.md files (excluding node_modules, .git, venv, etc.)
find . -name "ANTIGRAVITY.md" -type f \
...
**Target Selection:**
- Let users choose which ANTIGRAVITY.md file(s) to write to
- Use AI reasoning to suggest appropriate file based on learning content
- Global learnings (model names, general patterns) → `~/.antigravity/ANTIGRAVITY.md`
- Project-specific learnings → `./ANTIGRAVITY.md` or subdirectory file
...
- Once applied to ANTIGRAVITY.md, entries are permanent (edit manually to remove)
...
2. "Load learnings queue from ~/.antigravity/learnings-queue.json"
...
7. "Check for duplicates in existing ANTIGRAVITY.md"
8. "Present summary and get user decision"
9. "Apply changes to ANTIGRAVITY.md/AGENTS.md"
...
```json
{
  "todos": [
    ...
    {"content": "Load learnings queue", "status": "pending", "activeForm": "Loading queue from ~/.antigravity/learnings-queue.json"},
...
1. Find all ANTIGRAVITY.md files (global, project root, subdirectories)
...
# Example usage
count_lines("~/.antigravity/ANTIGRAVITY.md")
count_lines("./ANTIGRAVITY.md")
```
...
════════════════════════════════════════════════════════════
  ...
  ~/.antigravity/ANTIGRAVITY.md (global)           42 lines  ✓
  ./ANTIGRAVITY.md (project)                 156 lines  ⚠️
  ./src/ANTIGRAVITY.md (subdirectory)         28 lines  ✓

  AGENTS.md                              ✗ not found

────────────────────────────────────────────────────────────
⚠️  ./ANTIGRAVITY.md exceeds 150 lines
...
```bash
cat ~/.antigravity/learnings-queue.json | jq -r '.[] | "\(.timestamp) | conf:\(.confidence // 0.5) | decay:\(.decay_days // 90)d | \(.message | .[0:60])"'
```
...
Scan existing ANTIGRAVITY.md files for similar entries that could be consolidated.

**1. Read both ANTIGRAVITY.md files:**
```bash
cat ~/.antigravity/ANTIGRAVITY.md
cat ANTIGRAVITY.md 2>/dev/null
```
...
Group 2 (Project ANTIGRAVITY.md):
...
{
  "questions": [{
    "question": "Apply deduplication to ANTIGRAVITY.md files?",
    "header": "Dedupe",
    "multiSelect": false,
    "options": [
...
```bash
ls ~/.antigravity/projects/ | grep -i "$(basename "$(pwd)")"
```

2. Check if initialized (replace PROJECT_FOLDER with result from step 1):
```bash
test -f ~/.antigravity/projects/PROJECT_FOLDER/.reflect-initialized && echo "initialized" || echo "first-run"
```
...
2. **Handle underscores vs hyphens:** Directory names may use underscores (`darwin_new`) but encoded paths use hyphens (`darwin-new`). If first grep fails, try replacing underscores:
   ...
   ls ~/.antigravity/projects/ | grep -i "$(basename $(pwd) | tr '_' '-')"
   ```

3. Then list ALL session files in that folder:
   ```bash
   ls ~/.antigravity/projects/[PROJECT_FOLDER]/*.jsonl
   ```
...
**Key file structure:**
- Session files: `~/.antigravity/projects/[PROJECT_FOLDER]/*.jsonl`
...
```python
# c:/Users/user/Documents/GitHub/scrivener-flow/my-case-tracker/.agent/skills/antigravity-reflect/scripts/lib/semantic_detector.py
import sys
sys.path.append(r'c:\Users\user\Documents\GitHub\scrivener-flow\my-case-tracker\.agent\skills\antigravity-reflect\scripts')
...
- Provides cleaner `extracted_learning` for ANTIGRAVITY.md
...
```bash
python c:/Users/user/Documents/GitHub/scrivener-flow/my-case-tracker/.agent/skills/antigravity-reflect/scripts/extract_tool_errors.py --project "$(pwd)" --min-count 2 --json
```
...
- Read the queue from `~/.antigravity/learnings-queue.json`
...
- **Cleaner learnings** — extracts concise, actionable statements for ANTIGRAVITY.md
...
```python
# c:/Users/user/Documents/GitHub/scrivener-flow/my-case-tracker/.agent/skills/antigravity-reflect/scripts/lib/semantic_detector.py provides:
import sys
sys.path.append(r'c:\Users\user\Documents\GitHub\scrivener-flow\my-case-tracker\.agent\skills\antigravity-reflect\scripts')
...
3. If `is_learning == true`:
   ...
   - Add `extracted_learning` for cleaner ANTIGRAVITY.md entries
...
```bash
ls -lt ~/.antigravity/projects/ | grep -i "$(basename $(pwd))"
```

Then list files in that folder and pick the most recent non-agent file:
```bash
ls -lt ~/.antigravity/projects/[PROJECT_FOLDER]/*.jsonl | head -5
```
...
This should be offered as a skill improvement, not a ANTIGRAVITY.md entry.
```
...
- In Step 5, offer routing options: skill file | ANTIGRAVITY.md | both

**If NO skill context detected:**
- Continue with normal ANTIGRAVITY.md routing

### Step 3.5: Semantic Deduplication (Within Queue)

Before checking against ANTIGRAVITY.md, consolidate similar learnings within the current batch.
...
For each learning kept after filtering, search BOTH ANTIGRAVITY.md files:

```bash
grep -n -i "keyword" ~/.antigravity/ANTIGRAVITY.md
grep -n -i "keyword" ANTIGRAVITY.md
```

If duplicate found:
- Show: "⚠️ SIMILAR in [global/project] ANTIGRAVITY.md: Line [N]: [content]"
...
```json
{
  "questions": [{
    ...
    "options": [
      ...
      {"label": "ANTIGRAVITY.md", "description": "Add to project ANTIGRAVITY.md as general guidance"},
      {"label": "Both", "description": "Add to skill file AND ANTIGRAVITY.md"},
...
Duplicate check:
  ✓ None found
  OR
  ⚠️ SIMILAR in [global/project] ANTIGRAVITY.md:
...
**7a. Apply to ANTIGRAVITY.md (Primary Targets):**
1. Read current ANTIGRAVITY.md files
...
<!-- Auto-generated by antigravity-reflect. Do not edit this section manually. -->
...
<!-- End antigravity-reflect section -->
```

**Update Strategy:**
- Look for existing `<!-- Auto-generated by antigravity-reflect` marker
- If found: REPLACE the entire section (from marker to `<!-- End antigravity-reflect section -->`)
...
```bash
echo "[]" > ~/.antigravity/learnings-queue.json
```
...
════════════════════════════════════════════════════════════
  ✓ ~/.antigravity/ANTIGRAVITY.md    [N] entries
  ✓ ./ANTIGRAVITY.md            [N] entries
...
```bash
touch ~/.antigravity/projects/PROJECT_FOLDER/.reflect-initialized
```
...
If ANTIGRAVITY.md exceeds 150 lines, warn:
```
Note: ANTIGRAVITY.md is [N] lines. Consider consolidating entries.
```
