...
def get_queue_path() -> Path:
    ...
    return Path.home() / ".antigravity" / "learnings-queue.json"


def get_backup_dir() -> Path:
    """Get path to learnings backup directory."""
    return Path.home() / ".antigravity" / "learnings-backups"


def get_antigravity_dir() -> Path:
    """Get path to .antigravity directory."""
...
def find_antigravity_files(root_dir: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    Find all ANTIGRAVITY.md files in the project tree.

    Args:
        root_dir: Root directory to search from (defaults to cwd)

    Returns:
        List of dicts with {path, relative_path, type} for each ANTIGRAVITY.md found.
        Type is 'root', 'subdirectory', or 'global'.
    """
    root = Path(root_dir) if root_dir else Path.cwd()
    results = []

    # Always include global ANTIGRAVITY.md
    global_antigravity = get_antigravity_dir() / "ANTIGRAVITY.md"
    if global_antigravity.exists():
        results.append({
            "path": str(global_antigravity),
            "relative_path": "~/.antigravity/ANTIGRAVITY.md",
            "type": "global",
        })

    # Check root ANTIGRAVITY.md
    root_antigravity = root / "ANTIGRAVITY.md"
    if root_antigravity.exists():
        results.append({
            "path": str(root_antigravity),
            "relative_path": "./ANTIGRAVITY.md",
            "type": "root",
        })

    # Search for ANTIGRAVITY.md in subdirectories
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        if "ANTIGRAVITY.md" in filenames:
            full_path = Path(dirpath) / "ANTIGRAVITY.md"
...
def suggest_antigravity_file(learning: str, antigravity_files: List[Dict[str, Any]]) -> Optional[str]:
    """
    Suggest which ANTIGRAVITY.md file a learning should go to.

    This is a hint for Antigravity to use when reasoning about placement.
    ...
    if any(ind in learning_lower for ind in global_indicators):
        return "~/.antigravity/ANTIGRAVITY.md"

    # Check if learning mentions a specific directory
    for cf in antigravity_files:
