...
import { TrendingUp, Trophy, Users, Target, Zap } from 'lucide-react';
import { getWinRateData } from '@/app/actions/revenueLabActions';
import type { WinRateYearData } from '@/types/revenuelab';
import { WIN_RATE_LAB_YEARS } from '@/lib/investment/yearUtils';
import { MetricCard } from './win-rate/MetricCard';
import { StockListAccordion } from './win-rate/StockListAccordion';
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-wrap items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-[280px]">
                    ...
                    <input type="range" min={0} max={500} step={10} value={yoyLow} onChange={(e) => setYoyLow(Number(e.target.value))} className="flex-1 accent-indigo-500" />
                    ...
                    <input type="range" min={0} max={500} step={10} value={yoyHigh} onChange={(e) => setYoyHigh(Number(e.target.value))} className="flex-1 accent-indigo-500" />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <MetricCard icon={<Trophy className="w-4 h-4" />} label="最佳爆發次數" value={`${bestBucket.burstCount} 次`} sub={`勝率 ${bestBucket.winRate.toFixed(1)}%`} color="text-amber-600 dark:text-amber-400" />
                <MetricCard icon={<Target className="w-4 h-4" />} label="最高勝率" value={`${bestBucket.winRate.toFixed(1)}%`} sub={`${bestBucket.stockCount} 檔樣本`} color="text-emerald-600 dark:text-emerald-400" />
                <MetricCard icon={<TrendingUp className="w-4 h-4" />} label="最高平均漲幅" value={`+${highestAvgReturn.toFixed(1)}%`} sub={`${year} 年度`} color="text-indigo-600 dark:text-indigo-400" />
                <MetricCard icon={<Users className="w-4 h-4" />} label="總樣本股票數" value={`${totalStocks} 檔`} sub={`YOY ${yoyLow}-${yoyHigh}%`} color="text-slate-600 dark:text-slate-400" />
            </div>
            ...
            <Card>
                <CardHeader className="pb-2">
                    ...
                    <CardDescription>YOY {yoyLow}-{yoyHigh}% 達標次數與股價年度漲幅的關係</CardDescription>
                </CardHeader>
                <CardContent>
                            ...
                            <thead>
                                <tr className="border-b border-slate-200 dark:border-slate-700">
                                    {['爆發次數', '股票數', '平均漲幅', '中位數', '勝率(>20%)', '翻倍率(>100%)', '標準差'].map((h) => (
                                        <th key={h} className="py-2 px-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 whitespace-nowrap">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {buckets.map((b) => (
                                    <React.Fragment key={b.burstCount}>
                                        <tr className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            <td className="py-2 px-3"><Badge variant="outline" className="font-mono">{b.burstCount} 次</Badge></td>
