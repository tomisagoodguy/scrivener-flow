...
const MiniTaskCard = ({ task, onToggle, onDelete }: { task: TodoTask; onToggle: (id: string) => void; onDelete?: (id: string) => void }) => {
    return (
        <div className="group bg-white dark:bg-slate-800 p-2 rounded shadow-sm border border-slate-100 dark:border-slate-700 flex items-start gap-2 text-xs relative">
            ...
            <div className="flex-1 min-w-0">
                <div className={`truncate font-medium ${task.isCompleted ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>
                    {task.title}
                </div>
...
export function TodoMatrixView({ tasks, onToggle, onDelete }: Props) {
    ...
    const Quadrant = ({ title, desc, tasks, colorClass, bgClass }: { title: string, desc: string, tasks: TodoTask[], colorClass: string, bgClass: string }) => (
        <div className={`rounded-xl p-3 flex flex-col h-full overflow-hidden border ${colorClass} ${bgClass}`}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                {tasks.map(t => <MiniTaskCard key={t.id} task={t} onToggle={onToggle} onDelete={onDelete} />)}
                {tasks.length === 0 && (
