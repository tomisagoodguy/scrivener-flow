...
const TaskItem = ({ task, onToggle, onDelete }: { task: TodoTask; onToggle: (id: string) => void; onDelete: (id: string) => void }) => {
    const getTypeColor = (type: string) => {
        switch (type) {
            case 'legal': return 'bg-red-50 text-red-700 border-red-100 dark:bg-red-900/20 dark:text-red-300 dark:border-red-900/30';
            case 'tax': return 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-900/30';
            case 'appointment': return 'bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-900/30';
            default: return 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-900/30';
        }
    };
    ...
    const getTypeName = (type: string) => {
        switch (type) {
            ...
            case 'tax': return '稅單';
            case 'appointment': return '預約';
            default: return '個人';
        }
    };

    return (
        <div className={`group flex items-start gap-3 p-3 rounded-xl border transition-all ${task.isCompleted
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    <h4 className={`text-sm font-semibold whitespace-pre-wrap pr-2 ${task.isCompleted ? 'text-slate-500 line-through' : 'text-slate-800 dark:text-slate-200'}`}>
                        {task.title}
                    </h4>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded border ${getTypeColor(task.type)} whitespace-nowrap`}>
    ...
    return (
        <div className="space-y-6">
            ...
            {overdueTasks.length > 0 && (
                <section>
                    ...
                    <div className="space-y-2">
                        {overdueTasks.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDelete={onDelete} />)}
                    </div>
                </section>
            )}
            ...
            <section>
                ...
                <div className="space-y-2">
                    {todayTasks.length > 0 ? (
                        todayTasks.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDelete={onDelete} />)
                    ) : (
            ...
            <section>
                ...
                <div className="space-y-2">
                    {upcomingTasks.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDelete={onDelete} />)}
                </div>
            </section>
            ...
            {completedTasks.length > 0 && (
                <section className="opacity-75">
                    ...
                    <div className="space-y-2">
                        {completedTasks.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDelete={onDelete} />)}
                    </div>
                </section>
            )}
        </div>
