...
import { MonthCalendar } from './date-calculator/MonthCalendar';
...
export default function DashboardDateCalculator() {
    ...
    if (result && contractDate) {
        ...
        calendarMonths = months.filter((d) => d.getDate() === 1);
        milestones = [
            { date: parseISO(contractDate), label: '簽約', color: 'text-blue-600', bg: 'bg-blue-500 shadow-blue-200' },
            { date: parseISO(result.sign_diff_date), label: '補差', color: 'text-indigo-600', bg: 'bg-indigo-500 shadow-indigo-200' },
            { date: parseISO(result.seal_date), label: '用印', color: 'text-purple-600', bg: 'bg-purple-500 shadow-purple-200' },
            { date: parseISO(result.tax_payment_date), label: '完稅', color: 'text-pink-600', bg: 'bg-pink-500 shadow-pink-200' },
            { date: parseISO(result.handover_date), label: '交屋', color: 'text-emerald-600', bg: 'bg-emerald-500 shadow-emerald-200' },
        ];
    }
                ...
                <div className="space-y-6">
                    <div className="bg-gray-50 dark:bg-slate-950/40 p-5 rounded-2xl border border-gray-100 dark:border-slate-800">
                        <div className="mb-6">
                            ...
                            <input type="date" value={contractDate} onChange={(e) => setContractDate(e.target.value)} className="w-full bg-white dark:bg-slate-800 border-2 border-gray-100 dark:border-slate-700 rounded-xl px-4 py-3 focus:border-purple-500 outline-none transition-all font-bold text-gray-900 dark:text-white" />
                        </div>
                    ...
                    {result && (
                        <div className="animate-fade-in">
                            <div className="bg-gray-50 dark:bg-slate-950/40 rounded-2xl border border-gray-100 dark:border-slate-800 overflow-hidden mb-6 grid grid-cols-2 divide-x divide-y divide-gray-100 dark:divide-slate-800">
                                <div className="p-4"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">簽約後補差額</div><div className="font-bold text-indigo-600">{result.sign_diff_date}</div></div>
                                <div className="p-4"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">用印日期</div><div className="font-bold text-purple-600">{result.seal_date}</div></div>
                                <div className="p-4"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">預計完稅日</div><div className="font-bold text-pink-500">{result.tax_payment_date}</div></div>
                                <div className="p-4"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">預計交屋日</div><div className="font-bold text-emerald-600">{result.handover_date}</div></div>
                            </div>
                            <div className="space-y-6">
                                {calendarMonths.map((m, i) => <MonthCalendar key={i} monthStart={m} milestones={milestones} />)}
                            </div>
                        </div>
                    )}
                </div>
