...
export default function TimelineDashboard({ cases }: TimelineDashboardProps) {
    ...
    const { upcomingTasks } = useMemo(() => {
        ...
        cases.forEach((c) => {
            ...
            // 2. Check Todos List (New Custom Schedule/Notes)
            // Show ONLY manual/personal tasks here to avoid duplicating system reminders
            if (c.todos_list) {
                c.todos_list.forEach((todo) => {
                    ...
                    try {
                        ...
                        if (isWithinInterval(todoDate, { start: today, end: sevenDaysLater })) {
                            upcoming.push({
                                date: todoDate,
                                caseNumber: c.case_number,
                                buyer: todo.content, // Use content as primary labels
                        ...
                        console.error('Invalid todo date:', t.due_date);
                    }
                });
            }
        });
    ...
    // ... (Keep existing group by date logic)
    const days = useMemo(() => {
        const result = [];
        for (let i = 0; i <= 7; i++) {
...
function TaskCard({ task, isOverdue = false }: { task: TimelineItem, isOverdue?: boolean }) {
    return (
        <div
            ...
            `}
            title={`${task.caseNumber} - ${task.buyer}`}
        >
