...
- **Lesson**: For system-generated content, "Delete" must be a state (`is_deleted = true`), not a data removal. And the sync logic must respect this state—only creating _new_ ones if they truly didn't exist, and only "resurrecting" deleted ones if critical parameters (like the date) actually changed.
...
- **Mistake**: Replaced the existing `DashboardQuickNotes` with the new Todo feature on the homepage without verifying if the user wanted to _keep_ the old notes inside the new layout.
