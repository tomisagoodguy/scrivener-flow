...
class StrategyMessageBuilder(FlexMessageBuilder):
    """建構量化策略選股通知"""

    @classmethod
    def build(cls, strategy_name: str, changes: List[Dict[str, Any]], date_str: str) -> Dict[str, Any]:
        if not changes:
            return None
        
        # 分類變動
        new_buys = [c for c in changes if c['change_type'] == 'NEW_BUY']
        exits = [c for c in changes if c['change_type'] == 'EXIT']
        rebalance = [c for c in changes if c['change_type'] == 'REBALANCE']
        
        rows = []
        
        def _add_stock_list(title, items, color, icon):
            if not items: return
            
            rows.append(cls._text(f"{icon} {title}", weight="bold", color=color, size="sm", margin="md"))
            
            for item in items[:10]: # Limit 10
                rank_info = f"#{item.get('rank', 'N/A')}" if 'rank' in item else ""
                
                rows.append(cls._box_horizontal([
                    cls._text(f"{item['stock_name']}", size="sm", flex=4, color="#333333"),
                    cls._text(f"{item['stock_code']}", size="xs", color="#aaaaaa", flex=2, align="end"),
                    cls._text(rank_info, size="xs", align="end", flex=2, color=color)
                ], margin="sm"))
                
            if len(items) > 10:
                rows.append(cls._text(f"...還有 {len(items)-10} 檔", size="xs", color="#aaaaaa", align="end"))

        if new_buys:
            _add_stock_list("新增買入", new_buys, "#EF4444", "🚀")
            
        if exits:
            _add_stock_list("獲利/停損出場", exits, "#10B981", "💰")
            
        if rebalance:
            _add_stock_list("持股再平衡", rebalance, "#3B82F6", "⚖️")
        ...
        return {
            "type": "bubble",
            "header": cls._box_vertical([
                cls._text(f"🤖 {strategy_name}", weight="bold", size="lg", color="#ffffff"),
                cls._text(f"選股訊號 date: {date_str}", size="xs", color="#ffffffcc", margin="xs")
            ], backgroundColor="#6366f1", paddingAll="16px"),
            "body": cls._box_vertical(rows, paddingAll="16px"),
             "footer": cls._box_vertical([
                cls._button_uri("查看策略詳情", "https://scrivener-flow.vercel.app/investment", style="secondary")
            ])
        }
