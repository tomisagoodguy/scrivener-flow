...
def compute_liquidity(amt: pd.DataFrame, stock_ids: list, threshold: float) -> dict:
...
class StrategySignalStep(BaseStep):
    ...
    def execute(
        ...
        try:
            ...
            for strategy in ALL_STRATEGIES:
                try:
                    ...
                    if positions is None or positions.empty:
                        logger.warning(
                            f"[{strategy.strategy_id}] get_positions() returned empty"
                        )
                        continue
                    ...
                    logger.info(
                        f"[{strategy.strategy_id}] {len(rows)} stocks selected on {date_key}"
                    )
                    all_rows.extend(rows)

                except Exception as e:
                    logger.error(
                        f"[{strategy.strategy_id}] get_positions() failed: {e}"
                    )
            ...
            if FinlabDataError is not None and isinstance(e, FinlabDataError):
                ...
                logger.warning(
                    "[StrategySignalStep] FinLab quota exceeded, skipping strategy signals: %s",
                    e,
                )
        ...
        if all_rows:
            try:
                ...
                liquidity = compute_liquidity(
                    cache.amt, stock_ids, LIQUIDITY_TURNOVER_THRESHOLD
                )
                for row in all_rows:
                    avg_turnover, liquidity_flag = liquidity.get(
                        row["stock_id"], (None, None)
                    )
            ...
            new_codes = list(
                {row["stock_id"] for row in all_rows} - set(ctx.secondary_stock_codes)
            )
            if new_codes:
                ctx.secondary_stock_codes = ctx.secondary_stock_codes + new_codes
                logger.info(
                    f"[StrategySignalStep] 新增 {len(new_codes)} 支策略股至 secondary_stock_codes"
                )
    ...
    def _check_signal_staleness(
        ...
        系列策略的訊號日期軸對齊月營收揭露日（reindex 到 rev.index_str_to_date()），
        每月揭露完畢到下月揭露前有約 3 週的自然空窗期，判斷基準改以「距最新月營收
        揭露日的天數」為主，避免此空窗期被誤判為停更；取得揭露日失敗時回退為
        「距 server 當日的日曆天數」（沿用門檻 STALENESS_THRESHOLD_DAYS）。
        ...
        try:
            series_ids = [
                ...
                if s.strategy_id != _EXCLUDED_STRATEGY_ID
            ]
            ...
            latest_date = (
                ...
                else datetime.strptime(str(latest)[:10], "%Y-%m-%d").date()
            )
            disclosure_date = self._get_latest_revenue_disclosure_date()

            if disclosure_date is not None:
                if latest_date >= disclosure_date:
                    return
                stale_days = (disclosure_date - latest_date).days
                if stale_days > STALENESS_THRESHOLD_DAYS:
                    msg = (
                        f"策略訊號已停更 {stale_days} 天"
                        f"（距最新月營收揭露日 {disclosure_date.isoformat()}，"
    ...
    def _get_latest_revenue_disclosure_date(self) -> date | None:
        """回傳 monthly_revenue:當月營收 最新 index 日期（最新揭露日）。

        取得失敗（FinLab 例外、資料為空等）一律回傳 None，讓呼叫端回退為
        距 server 當日的日曆天數判斷，不因此中斷停更檢查。
        """
        try:
            ...
            logger.warning(
                "[StrategySignalStep] 取得月營收最新揭露日失敗，回退距今日曆天判斷: %s",
                e,
            )
            return None
