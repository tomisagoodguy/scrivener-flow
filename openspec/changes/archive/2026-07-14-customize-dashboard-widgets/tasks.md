...
- [x] 6.2 驗證完整互動流程（對應「Widget Reordering via Drag and Drop」的 reordering three widgets 範例）：因本機瀏覽器自動化工具（playwright profile）被殘留 lockfile 卡住、且經使用者確認不強制清除，改以下列方式驗證：(1) 逐檔 code review `WorkDashboard.tsx`/`DashboardWidgetShell.tsx`/`HiddenWidgetsMenu.tsx`/`useDashboardLayout.ts`/`dashboardLayout.ts` 邏輯與 design.md 情境逐條比對（welcome-header 不受控制、hideWidget/showWidget 插入 order 最大值+1、reorderWidgets 重新指派可見清單 order 0..n-1、樂觀更新失敗回滾），(2) 29 個單元測試（`layoutTypes`/`dashboardLayout`/`useDashboardLayout`/`DashboardWidgetShell`/`HiddenWidgetsMenu`）全綠，(3) `yarn tsc --noEmit` 無錯誤，(4) 將 `supabase/migrations/20260714120000_add_dashboard_layout.sql` 實際套用到遠端專案（`my-case-tracker`），確認 `user_settings.dashboard_layout` 欄位存在且原有 4 條 RLS policy 未變動。裝置模擬觸控拖曳與跨瀏覽器 session 的即時互動仍待使用者日後於瀏覽器手動操作確認。
